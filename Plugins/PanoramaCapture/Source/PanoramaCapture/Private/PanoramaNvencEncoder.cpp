...
#endif
{
    LifetimeStats = FPanoNvencEncoderLifetimeStats();
}
...
void FPanoNvencEncoder::GetLifetimeStats(FPanoNvencEncoderLifetimeStats& OutStats) const
{
    FScopeLock Lock(&StatsGuard);
    OutStats = LifetimeStats;
}
...
bool FPanoNvencEncoder::EncodeFrame_RenderThread(FTextureRHIRef Texture, uint64 InFrameIndex, double Timecode)
{
    ...
    if (!EncoderInput.IsValid())
    {
        ...
        ](const FVideoEncoder::FEncodedImage& EncodedImage)
        {
            ...
            RecordEncodedBytes(EncodedImage.Data.Num());
    ...
    if (!EncoderInput.IsValid() || !Encoder.IsValid())
    {
        return false;
    }

    const double EncodeStartTime = FPlatformTime::Seconds();
    ...
    RecordEncodeDuration((EncodeEndTime - EncodeStartTime) * 1000.0);

    return true;
}
#endif

void FPanoNvencEncoder::RecordEncodeDuration(double Milliseconds)
{
    FScopeLock Lock(&StatsGuard);
    ++LifetimeStats.FramesEncoded;
    LifetimeStats.TotalEncodeTimeMs += Milliseconds;
    LifetimeStats.MaxEncodeTimeMs = FMath::Max(LifetimeStats.MaxEncodeTimeMs, Milliseconds);
}

void FPanoNvencEncoder::RecordEncodedBytes(uint64 NumBytes)
{
    if (NumBytes == 0)
    {
        return;
    ...
    FScopeLock Lock(&StatsGuard);
    LifetimeStats.TotalEncodedBytes += NumBytes;
}
